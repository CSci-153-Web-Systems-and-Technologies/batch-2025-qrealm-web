...
import { EventQRGenerator, QRCodeOptions } from '@/lib/qr-generator'
...
export function QRCodeDisplay({ qrCodeUrl, eventCode, eventTitle, eventData = {} }: QRCodeDisplayProps) {
  ...
  try {
    ...
    const filename = `qrealm-${eventCode}-${eventTitle.replace(/\s+/g, '-').toLowerCase()}${highRes ? '-highres' : ''}.png`;
    
    // Simple browser-compatible method
    const canvas = document.createElement('canvas');
    await QRCode.toCanvas(canvas, `${window.location.origin}/event/${eventCode}`, {
      width,
      margin: 2,
    ...
    canvas.toBlob((blob) => {
      ...
      showSuccessToast(`QR code ${highRes ? '(high-res) ' : ''}downloaded successfully!`);
  ...
  const handleDownloadSVG = async () => {
    ...
    try {
      ...
      a.style.display = 'none';
      a.href = url;
      a.download = `qrealm-${eventCode}-${eventTitle.replace(/\s+/g, '-').toLowerCase()}.svg`;
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        img.onload = () => resolve()
        img.onerror = () => reject(new Error('Failed to load QR image for PDF (CORS or 404)'))
      })
      ...
      try {
        ...
        const filename = `qrealm-${eventCode}-${eventTitle.replace(/\s+/g, '-').toLowerCase()}.pdf`
        pdf.save(filename)
        ...
        console.warn('jspdf not available or failed, falling back to print window:', err)
        const html = `
          <html>
            <head>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <Button variant="outline" size="sm" onClick={() => handleDownloadPNG(true)}
                  disabled={isDownloading}
                  >
